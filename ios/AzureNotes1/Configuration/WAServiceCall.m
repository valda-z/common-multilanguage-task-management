...
+(WAServiceRequest *)createServiceRequestWithURL:(NSString *)url httpMethod:(NSString *)httpMethod payload:(NSData *)payload contentType:(NSString *)contentType withCompletionHandler:(void ( ^ )(NSInteger statusCode, NSData* response, NSError* error))block;

+(WAServiceRequest *)createServiceRequestWithURL:(NSString *)url withCompletionHandler:(void ( ^ )(NSInteger statusCode, NSData *response, NSError *error))block;

@end

@implementation WAServiceCall
...
+ (NSString *)iso8601StringFromDate:(NSDate *)date
{
    ...
    if (!sISO8601) {
		...
		NSMutableString *strFormat =[NSMutableString stringWithString:@"yyyy-MM-dd'T'HH:mm:ss.sss'Z'"];[sISO8601 setTimeStyle:NSDateFormatterFullStyle];[sISO8601 setDateFormat:strFormat];
    }
...
{
	WAServiceRequest *request =[WAServiceRequest createServiceRequestWithURL:urlStr 
...
{[self getFromURL:urlStr withXmlCompletionHandler: ^ (xmlDocPtr doc, NSError * error) {
		 ...
		 for (xmlNodePtr child = root->children; child; child = child->next) {
			 ...
			 NSString *value =[NSString stringWithUTF8String:(char *)xmlValue];
			 xmlFree(xmlValue);[dict setObject:value forKey:name];
		 }
...
@implementation WAServiceRequest - (id)initServiceRequestWithURL:(NSString *)url httpMethod:(NSString *)httpMethod payload:(NSData *)payload contentType:(NSString *)contentType withCompletionHandler:(void ( ^ )(NSInteger statusCode, NSData *response, NSError *error))block
{
	if ((self =[super init])) {
		_block =[block copy];
		
		_request =[NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];[_request setHTTPMethod:httpMethod];[_request setHTTPShouldHandleCookies:NO];[_request setValue:@"Microsoft ADO.NET Data Services" forHTTPHeaderField:@"User-Agent"];
		
		if (payload) {[_request setHTTPBody:payload];[_request setValue:contentType forHTTPHeaderField:@"Content-Type"];
		}
		
		WACloudAccessToken *token =[WACloudAccessControlClient sharedToken];
		if (token) {[token signRequest:_request];
		}
		
		LOG(@"%@ %@", httpMethod, url);
		LOG(@"Headers: %@",[_request allHTTPHeaderFields]);
	}
...
{
    return[[self alloc] initServiceRequestWithURL:url httpMethod:httpMethod payload:payload contentType:contentType withCompletionHandler:block];
} + (WAServiceRequest *)createServiceRequestWithURL:(NSString *)url withCompletionHandler:(void ( ^ )(NSInteger statusCode, NSData *response, NSError *error))block
{
	return[[self alloc] initServiceRequestWithURL:url httpMethod:@"GET" payload:nil contentType:nil withCompletionHandler:block];
